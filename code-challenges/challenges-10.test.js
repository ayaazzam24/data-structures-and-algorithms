...
const getNames = (arr) => {
  // Solution code here...
  return arr.map((value,inx)=>value.name.split('').reverse().join('')); 
};
...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((acc,value,idx)=>{
    acc = acc + value.reduce((acc , val ,indx)=>{
      if (val === target){
      acc = acc+1
    }
    return acc;
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let array = input.map(element => element.filter (value => typeof(value) === 'number' && !(value % 5))); 
  let numberS = array.map(val => val.map(elem => Math.pow(2,elem)));
  return numberS;
};
...
let findShortest = (data) => {
  // Solution code here...
  let shortT = data.sort((ele1,ele2) => ele1.height - ele2.height );
  return shortT[0].name; 
};
